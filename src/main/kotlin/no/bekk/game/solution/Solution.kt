...
class DodgeFallingSqueresSolution: AppModule {
    ...
    private fun handlePlayerMovement(delta: Float) {
        ...
        if (player.position.x > EngineConfig.VIEWPORT_WIDTH) { player.position.x = EngineConfig.VIEWPORT_WIDTH }

        if (player.position.y < 0) { player.position.y = 0f }
        if (player.position.y > EngineConfig.VIEWPORT_HEIGHT) { player.position.y = EngineConfig.VIEWPORT_HEIGHT }
    }
